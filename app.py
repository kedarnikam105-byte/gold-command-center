...
# Chief Officer Final Decision Highlight
if agents["chief"].last_report:
    st.divider()
    st.subheader("🎯 Chief Officer — Final Recommendation")
    ...
    st.markdown(f"""
    <div style="background:#0d1b2a; border:2px solid {color}; border-radius:16px; padding:24px; text-align:center;">
        <h2 style="color:{color}; margin:0;">{action.upper()}</h2>
        <p style="font-size:1.3em; margin:8px 0;">Confidence: {cr.get('confidence', 0)}%</p>
        <p style="color:#ccc;">{cr.get('summary', '')}</p>
    </div>
    """, unsafe_allow_html=True)
